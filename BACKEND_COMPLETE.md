...
- Построение мультиточечных маршрутов
- Расчёт ETA с учётом разгрузки
...
Backend полностью готов к подключению фронтенда. Все endpoints работают и задокументированы в `server/README.md`.

Для полной работы внешних API необходимо:
1. Добавить `GEMINI_API_KEY` в `.env`
2. Добавить `YANDEX_MAPS_API_KEY` в `.env`
