...
Правила:
1. Суммарный вес всех грузов в фуре НЕ должен превышать capacity_weight этой фуры. НИКОГДА не превышай этот лимит.
2. Суммарный объём в фуре НЕ должен превышать capacity_volume (если capacity_volume > 0). НИКОГДА не превышай этот лимит.
3. Приоритет отдавай заказам с более ранней датой created_at — они должны попасть в фуру в первую очередь.
4. Группируй грузы по схожим направлениям маршрута.
5. Максимизируй загрузку фур.
6. Учитывай actual_weight (фактический вес) вместо weight, если он задан.
7. Если груз не умещается ни в одну фуру — только тогда помести в unassigned.
